...
@app.command()
def main(
    dataset_path: Annotated[Path, typer.Option(help="The dataset path")] = PROCESSED_DATA_DIR / "stock_data_1.0.0.parquet",
    freq: Annotated [str, typer.Option(help="The daily frequency (e.g. 30D) for graph snapshot updating")] = '30D',
    output_path: Annotated[Path,  typer.Option(help="The output path where graph snapshots are saved")] = PROCESSED_DATA_DIR,
):
    """
    ...
    # Log info
    logger.info(f"Build graph snapshots with {freq} freq for {total_intervals} timestamps starting from {date_ranges[-1].strftime('%Y-%m-%d')}...")
    ...
    for i in range(len(date_ranges) - 1, 0, -1):
        ...
        logger.info(f"Iteration {iteration_count}/{total_intervals} completed. Time for this iteration: {iteration_time:.2f} seconds")
