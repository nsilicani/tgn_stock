...
class Data:
    ...
    def compute_target(
        self, df: pd.DataFrame, strategy: str = "binary"
    ) -> pd.DataFrame:
        if strategy == "binary":
            pass
    ...
    def compute_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["CCI"] = ta.cci(
            high=df["High"], low=df["Low"], close=df["Adj Close"], length=20
        )
        ...
        stoch = ta.stoch(
            high=df["High"], low=df["Low"], close=df["Adj Close"], k=14, d=3, smooth_k=3
        )
